...
#   ## 3.3 react query results 
  ...
  numbers <- reactive({
    numbers <- general() %>% nrow()
    numbers
  }) 
  
  output$rowNumbers <- renderText({ paste0("We have as many studies", numbers() )
  })
  ...
  result2 <- reactive({
    ...
    left_join(select(populations(), popSize, PMID), by="PMID") %>%
...
#   ##### Quelques nouvelle variable
  ...
  population <- reactive({
    population <- populations() %>% left_join(select(general(), author, PMID ), by ="PMID" ) %>%
...
#   ## Reactive results filtered on SelectedPMID ####
  ...
  # output$tableTest <-  DT::renderDataTable(DT::datatable(data() %>%
  #                                                             select(PMID)))
