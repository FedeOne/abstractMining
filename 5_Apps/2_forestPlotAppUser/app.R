...
load_all <- function(){
  ...
  runStatementRes <- dbSendQuery(connection, "SELECT * FROM results")
...
frequencies <- frequencies2 %>% 
  ...
  left_join(select(general,"AUTHOR" =  author, idStudy), by ="idStudy") %>% 
  left_join(select(population2, idPopulation, popCountry, popDisease, popComorbidity ), by = "idPopulation")
...
result2 <- result %>% 
  ...
  left_join(select(general, idStudy,author, yearPublication,studyPlan), by = "idStudy") %>% 
  rename("Author" = "author") %>% 
  left_join(select(population2, popSize, idPopulation), by="idPopulation") %>% 
  ...
  final2 <- result2 %>% 
    ...
    rename("REFERENCE" = refExp1,
           "N" = pop_size,
           "OUTCOME" = outcome) %>% 
    filter(icLow!= 0 |icUpper!=0, !is.na(icLow)| !is.na(icUpper))
  ...
  population <- population2 %>% left_join(select(general, author, idStudy ), by ="idStudy" ) %>% 
    mutate(across(c(ageMean, ageSD, ageLow, ageUp,popSize,femaleFreq) , as.numeric),
           ...
           femaleFreq = ifelse(femaleFreq == 0, NA, femaleFreq)
           ) %>% 
...
server = function(input, output, session) { 
  ...
  observe({
    updateSelectInput(session, "Columns", choices = list("EXPOSURE","SUBGROUP","RESULTS","OUTCOME","AUTHOR","N","REFERENCE","MEASURE","AuthorYear","EXP_num" ),
                      selected = c("AUTHOR","EXPOSURE","RESULTS","OUTCOME"))
  })
  
  observe({
    updateSelectInput(session, "userID", choices = final$user_id, selected = head(final$user_id, 1))
  })
  ...
  data <-  reactive(
    final %>% mutate(
      ...
      EXPOSURE = ifelse(is.na(expLow1) & is.na(expHigh1), paste0(exp1),
                        ifelse(is.na(expLow1) & !is.na(expHigh1), paste0(exp1,"<=", expHigh1," ",expUnit1), # exp = TSAT <= 15 %
                               ifelse(is.na(expHigh1) & !is.na(expLow1), paste0(exp1, ">=", expLow1," ",expUnit1), # Exp = TSAT >= 15 %
                                      ifelse(!is.na(expHigh1) & !is.na(expLow1) & expHigh1 == expLow1, paste(exp1,"=",expLow1," ",expUnit1),
                                             paste0(exp1," ",expLow1 ,";",expHigh1 ," ",expUnit1) )))),
     
       EXP_num = ifelse(is.na(expLow1) & is.na(expHigh1), paste0(exp1),
                       ifelse(is.na(expLow1) & !is.na(expHigh1), paste0(exp1,"<=", expHigh1," ",expUnit1), # exp = TSAT <= 15 %
                              ifelse(is.na(expHigh1) & !is.na(expLow1), paste0(exp1, ">=", expLow1," ",expUnit1), # Exp = TSAT >= 15 %
                                     ifelse(!is.na(expHigh1) & !is.na(expLow1) & expHigh1 == expLow1, paste0(exp1,"=",expLow1," ",expUnit1),
                                            paste(exp1," ",round(expLow1,input$round) ,";", round(expHigh1,input$round) ," ",expUnit1)))))
        ) %>% 
      filter(str_detect(tolower(exp1),input$expo1)  & str_detect(tolower(OUTCOME), input$outcome)) %>%
      filter(No %notin% input$exclude, user_id %in% input$userID, idProject %in% input$projectID)
 ...
 # Forest plot ----
  ...
  myPlot <- function(){
    ...
    ## GGplot axis limits
    # if(min(data()$icLow, na.rm = TRUE) < 1){
    #   minXaxis <-  min(data()$icLow, na.rm = TRUE)
    # } else{
    #   minXaxis <- 0.5
    # }
    # 
    # if(max(data()$icUpper, na.rm = TRUE) > 3){
    #   maxXaxis <- 3
    # } else{
    #   maxXaxis <- max(data()$icUpper, na.rm = TRUE)
    # }
    # 
    minXaxis <- 0.5
    maxXaxis <- 3
    ...
    f <- ggplot(ES()) + 
      ...
      theme_classic()  +
      scale_x_continuous(limits=c(minXaxis,  maxXaxis) )+
    ...
    a <- ggplot(data=data(), aes(y= No, x = Result, xmin=icLow, xmax=icUpper)) +
      ...
      scale_x_continuous(limits=c(minXaxis,  maxXaxis), position = "top") + 
      scale_y_discrete("N") +
      geom_vline(xintercept=1, color="black", linetype="dashed", alpha=.5) + 
      theme_classic() +
      theme(axis.title.y=element_blank(),
...
funnel_fun <-  function() {
  ...
  output$tableDT <-  DT::renderDataTable(
    DT::datatable(general %>% 
                    select( idStudy, PMID, author, yearPublication, title,objective,
                           studyPlan
                    ) %>% 
                   filter(idStudy %in% data()$idStudy))  )
    
  output$tableAbstract <-  DT::renderDataTable(DT::datatable(general %>% select(idStudy, PMID,author, ABSTRACT) %>% 
                                                           filter(idStudy %in% data()$idStudy))  )
  ...
  output$boxplot = renderPlot({ 
    ggplot(pop_reac(),aes(x=Author, color = popDisease)) + 
...
ui = navbarPage(theme =shinytheme("sandstone"),  ### navbar pour la barre de navigation
                ...
                tabPanel(h5("Make your forest plot"),   ### on insere le premier panneau de la barre de navigation et son titre
                         ...
                         column(3, wellPanel(div(class = "row",
                         ...
                         column(2,wellPanel( 
                           div(
                            selectInput(inputId = "userID",label = "User ID",choices = "Names" ,width = '80px', multiple = TRUE ),
                            textInput(inputId = "projectID",label = "Project ID", width = '80px'),
                         ...
                         column(2,wellPanel(numericInput(inputId = "X_break_5",
                                            ...
                                            numericInput(inputId = "X_break_7",
                                                         ...
                                                         value = 3.5))),
                         column(3, wellPanel(selectInput(inputId = "studyType",
                                                         label = "Filter for study plans",
                                                         choices = c("meta analysis", "cohort", "clinical trial","other"  ),
                                                         selected = c("meta analysis", "cohort", "clinical trial","other" ),
                                                         multiple = TRUE
                                                         ),
          ...
          # Pop desc tab ---- 
                ...
                tabPanel(h5("Population description"),
                         ...
                         selectInput(inputId = "sexFill",
                                     label = "Colour",
                                     choices= c("popDisease","ageClass","sexClass"),
                                     selected = "popDisease"),
