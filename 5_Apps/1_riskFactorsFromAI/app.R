...
server = function(input, output, session) { 
  ...
  query <- reactive({
    paste0("SELECT PMID, author, title, studyPlan, yearPublication, projectName, journalShort 
...
# list open connections ----
 ...
 results2 <- reactive({ 
   results2 <- results() %>% 
     ...
     left_join(select(general(), PMID, author, studyPlan, yearPublication, journalShort), by = "PMID") %>% 
 ...
 ## 1.2.1: Prepare Db for journal plot
 
 get20Journals <-  reactive({
   get20Journals <- results2() %>% 
 ...
 journalPlotDb <- reactive({
   journalPlotDb <- results2() %>% 
     filter(PMID %in% selectedPMID()$PMID ) %>% 
 ...
 ## 1.2.2: Prepare for measure per year plot
 
 pubsPerYear <-  reactive({
   pubsPerYear <- results2() %>% 
     select(PMID, measureType, yearPublication) %>% 
     filter(PMID %in% selectedPMID()$PMID ) %>% 
 ...
 yearByMeasure <- reactive({ 
     yearByMeasure <- results2() %>%  
       select(PMID, measureType, yearPublication) %>% 
       filter(PMID %in% selectedPMID()$PMID ) %>% 
       filter(measureType != "pValue") %>% 
       group_by(yearPublication) %>% 
 ...
 studyYearPlot <- function(){
    ...
    study <-  selectedPMID() %>% 
     ...
     labs(y = paste0("count"), title = "Type of selected studies") 
   
   
   journalPlot <- journalPlotDb() %>% 
   ...
   year <- selectedPMID() %>% 
     ...
     geom_bar() +
     scale_x_continuous(breaks = seq(min(selectedPMID()$yearPublication), max(selectedPMID()$yearPublication), 1)) +
     theme_classic() +
     theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
     labs(x = paste0("Year of publication"), title = "Published studies per year")
   
    
   yearMeasurePlot <- yearByMeasure() %>% 
     ggplot(aes(x = yearPublication, y = measureTypeOnPubsPerYear, fill = measureType)) +
     geom_col() +
    ...
    labs(title = "Type of analysis over time adjusted on publications/year")
   
   ggarrange(study, journalPlot, year, yearMeasurePlot, ncol = 2, nrow = 2)
 ...
 # output$tableYear <- DT::renderDataTable(DT::datatable( yearByMeasure()) )
...
ui = navbarPage(
  ...
  tabPanel(h5("Research equation"),
           ...
           br(),
           DT::dataTableOutput("tableYear"),
           br(),
           plotOutput("sutdyYearPlot", width = "100%",height = "800px"),
